...
- **Пользователи и роли** («Настройки» → «Пользователи панели», только админ): **админ** — всё; **модератор** — просмотр, скриншот, перезапуск игры и вход (как модератор в Telegram), бан/разбан, «Нарушения» и «Твинки»; **наблюдатель** — только просмотр (без паролей, приватов и IP игроков, без рецептов микстур/кулинарии). Права проверяются сервером на каждый запрос; добавление/роль/сброс/удаление — под паролем, в аудит; новый пользователь меняет пароль при первом входе; последнего админа удалить нельзя. Старый `webui_auth.json` с одним пользователем сам становится админом.
...
**Главная** (до входа): то же, что вкладка «Сервер» — статус, онлайн и график за сутки, сезонный рейтинг, торговцы, кланы, кто сейчас на сервере (`/api/public`, без входа, кэш 60 с) — и форма входа. Вкладки: **Профиль** (уровень, статы, исследования, способности, где я и мои территории с названиями карт, склад и инвентарь, друзья с уровнем и кланом), **История** (мои графики уровня, рейтинга, техов, наигранного — панель пишет почасовой снимок `logs\player_points.jsonl`, техи восстанавливаются задним числом из журнала трекинга), **Изучение** (схема технологий, клик по узлу — путь и сколько часов осталось), **Крафт** (раскладка до сырья с отметкой своих техов), **Справочник** (свойства предмета, рецепт/станок, нужная технология, куда идёт дальше, сколько лотов на рынке), **Рынок** (моя торговля — терминал, магазины, свои лоты; поиск по всем терминалам и магазинам, курс, сводка цен — без выручки и складов чужих продавцов; собирается в фоне раз в 5 мин), **Карта** (свои участки и «вы здесь» поверх карты с туманом войны: видно 20 клеток вокруг персонажа, своих участков и мест, где персонаж уже бывал — `playerweb_explored.json`, пополняется раз в 2 мин по онлайн-игрокам; туман накладывается на сервере, целая карта наружу не уходит; поворот как в админке, протяжка, масштаб колесом), **Сундуки** (что лежит в хранилищах на своих участках и брошенное на землю, поиск предмета, итог по всем участкам; природные клады под землёй не показываются), **Клан** (состав, роли, специализации, клан-техи, рост и события — без личных техов/исследований участников), **Чат** (только чтение, как в мессенджере — новые снизу, при прокрутке вверх догружаются старые, новые подтягиваются раз в 20 с: общий, английский, русский, клановый — сообщения нынешних участников своего клана; личные — список бесед и переписка с каждым собеседником, только свои), **Сервер** (статус, онлайн за 24 ч, сезонный рейтинг, торговцы, кланы, кто сейчас на сервере). Иконки предметов — из атласа `item_icons.png/.json` в каталоге панели; его собирает `build_item_icons.py` из распакованной графики игры (`Assets/Art/Items`) — сама графика принадлежит игре и в репозиторий не входит, без атласа панель работает без иконок. Галочка «Запомнить меня» — вход на 30 дней (на диске только хэш токена, гаснет при смене пароля в игре). **Вход в админку для стаффа** (`playerweb.admin_proxy`): игрокам с ролью в игре модератор/админ/мастер рядом с языком показывается кнопка «Админка»; при первом нажатии задаётся отдельный админский пароль (не игровой, от 8 символов), дальше вход по нему. Админка открывается на том же порту по `/admin/` (панель игроков передаёт запросы админке внутри процесса); роль в админке — по роли в игре (модератор → модератор, админ/мастер → админ), перепроверяется раз в минуту — сняли роль в игре, доступ пропал. На порту 80 `/admin/` отвечает только при действующей сессии админки с ролью (её выдаёт вход через кнопку «Админка»); всем остальным — 403, формы входа там нет. Вёрстка рассчитана и на телефон. Интерфейс на русском и английском (переключатель RU/EN в шапке, по умолчанию — по языку браузера); английские названия предметов, способностей и описания — из клиентской локализации игры (`game_i18n.py`), названия технологий переводятся через названия предметов; ники, кланы и тексты чата не переводятся. В справочнике и крафте предметы — кнопки: крафт ↔ справочник ↔ рынок.

Только чтение, чужих инвентарей, IP, приватов и паролей в ответах нет. `playerweb.allowed_nets` пусто = пускать всех.

**HTTPS** («Настройки» → «HTTPS панели игроков», `playerweb.https`, применяется без перезапуска): при включении панель работает только по `https://<домен>` — запросы по http и по IP получают 301 на домен. Два режима:
- **за обратным прокси** (по умолчанию): TLS и сертификат (например, certbot) — на прокси; прокси передаёт `X-Forwarded-Proto` и `X-Forwarded-For`, его адрес вписывается в «Доверенные прокси» (`playerweb.trusted_proxies`) — иначе заголовкам не верим. Путь `/.well-known/acme-challenge/` прокси отдаёт сам, не бэкенду;
- **свой сертификат** (`builtin`, без прокси): панель сама получает сертификат Let's Encrypt (`acmecert.py`, ACME v2, проверка HTTP-01 на порту панели), слушает `https.port` (443) и продлевает сертификат за 30 дней до конца; файлы — в `certs\`. Снаружи должны быть открыты 80 и 443 прямо на сервер. Для отладки — `staging` (тестовый сертификат без лимитов Let's Encrypt).

По HTTPS куки входа уходят с флагом `Secure`.

### Ротация бэкапов мира

Игра сама пишет архив мира `<мир>\backup\arhN.zip` раз в `timeBackupServer` секунд (`Data\config.json` мира; 1800 = раз в 30 мин, ~100 МБ каждый) и старые не удаляет — на живом сервере это несколько ГБ в сутки. `players.backup_rotation` («Настройки» → «Данные локального сервера»): хранятся `keep_recent` свежих архивов (48 = сутки) и по одному в день за `keep_days` дней, остальные удаляются, оставшиеся перенумеровываются подряд `arh0..arhN` (чтобы игра не перезаписала сохранённый архив, как бы она ни выбирала следующий номер). Запускается не чаще раза в час и только через 2–20 мин после свежего архива. По умолчанию выключено.
