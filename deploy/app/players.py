...
def star_name(cfg, star_id):
    ...
    int32 len, имя. У только что добавленной обновлением игры системы имя может быть пустым (игра даёт его позже) -> None."""
    wd = find_world_dir(cfg)
    if not wd:
        return None
...
def _parse_star_file(path):
    ...
    # если распознан, иначе порядковый номер. Порядковый номер НЕНАДЁЖЕН: 25.09 на рабочем сервере
